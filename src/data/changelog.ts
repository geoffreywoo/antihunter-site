...
export const changelog: ChangelogEntry[] = [
	{
		...
		summary:
			'prepared daily narrative rollup and execution receipts for antihunter.com. Nightly rollup (2026-03-05): On antihunter.com, execution stayed in continuity mode: 2 commits kept treasury/winner state and public changelog cadence current (switch to repo-wide narrative rollups and rewrite 2026-03-04 story and nightly rollup). Across clawfable, the day was a substantive product + infrastructure sprint (9 commits): the onboarding/protocol surface was tightened via remove revise mode from API, all-forks model, remove reviseArtifact, all-forks model, protect openclaw-template from API revise (admin token bypass only), and skill v1.4.0: default upload is fork, add artifact URL guidance, protect baseline from revise; core reliability was hardened with allow verify without explicit tweet_url by falling through to search-based verification; brand/distribution touchpoints were improved through add twitter:site meta tag for @clawfable, tag @clawfable in verification tweet text, and add @clawfable X link to footer.',
		links: [],
	},
	{
		day: 26,
